...
> 原文：<https://blog.devgenius.io/how-to-partition-k-equal-sum-subsets-d9ae843bb03c?source=collection_archive---------20----------------------->

## 第 19 天——100 天到 LinkedIn、雅虎、甲骨文

![](img/d37c00880906b746e6053469d83bb078.png)

照片由来自 [Pexels](https://www.pexels.com/photo/locker-room-in-sports-club-3875521/?utm_content=attributionCopyText&utm_medium=referral&utm_source=pexels) 的波琳娜·坦基列维奇[拍摄](https://www.pexels.com/@polina-tankilevitch?utm_content=attributionCopyText&utm_medium=referral&utm_source=pexels)

*   出于免费的故事？下面是我的 [**好友链接。**](https://medium.com/dev-genius/how-to-partition-k-equal-sum-subsets-d9ae843bb03c?source=friends_link&sk=01f62917842df1dabe5b117bbfd5832c)
*   100 天到 LinkedIn，雅虎，甲骨文

# Introduction🛹

嘿，伙计们，今天是 LinkedIn 挑战 100 天的第 19 天。
...
给定一个整数数组`nums`和一个正整数`k`，找出是否有可能将这个数组划分为`k`个和均相等的非空子集。
...
1.  首先我们要对数组进行排序。
2.  求数组中所有元素的和。如果总和不能被 k 整除，那么我们就不能分割成 k 个不同元素，所以返回 false。
3.  现在你必须递归地，用 k 个分区，我们可以划分数组的索引范围，剩余的总和，以及索引是否被访问过。
4.  对于每次递归，检查当前目标和是否可以达到，如果可以，则返回 true。
5.  如果无法达到目标总和，或者您只能通过已经访问过的索引来达到目标。返回 false🔚

# 使用 Chakra UI Vue 的 UI 开发—图标按钮

> 原文：<https://blog.devgenius.io/ui-development-with-chakra-ui-vue-icon-buttons-d2aaa143cbcc?source=collection_archive---------5----------------------->

![](img/4deae2ca8df1eb41e0dfa96352a065e8.png)
...
import { CBox, CIconButton } from "@chakra-ui/vue";export default {
  components: {
    CBox,
    CIconButton,
  },
...
我们添加了`icon`属性来设置要显示的图标的名称。

`aria-label`有图标的文字描述。

`variant-color`道具设置背景颜色:
...
`customIcons`拥有 SVG 图标的`path`元素。

我们将`customIcons`合并到`extend`来添加图标。

我们通过将`icon`属性设置为`customIcon`中图标的属性名来使用图标。

# 结论

我们可以用 Chakra UI Vue 轻松添加图标按钮。
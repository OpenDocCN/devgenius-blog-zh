...
> 原文：<https://blog.devgenius.io/how-to-solve-increasing-triplet-sequence-25d503c11189?source=collection_archive---------10----------------------->

## 第 27 天——100 天到 LinkedIn、雅虎、甲骨文

![](img/ad2fd824c847c5fb0f3f42c730f9b673.png)

斯蒂芬·亨宁在 [Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral) 上的照片

*   出免费故事？这里是我的 [**好友链接。**](https://medium.com/@akshay_ravindran/25d503c11189?source=friends_link&sk=4e95dbaa0e118b8dfcb8eb506bd3a477)
*   100 天到 LinkedIn，雅虎，甲骨文

# Introduction🛹

嘿，伙计们，今天是 LinkedIn 挑战 100 天的第 27 天。
...
在我们进入问题之前，如果你想知道我为什么选择 LinkedIn、雅虎和甲骨文而不是 FAANG，是因为我已经完成了一项挑战[重点是亚马逊和脸书的面试](https://medium.com/javarevisited/100-days-to-amazon-day-1-b9e07228f079)。
...
给定一个未排序的数组，返回数组中是否存在长度为 3 的递增子序列。
...
**注:**你的算法要在 O( *n* )时间复杂度和 O( *1* )空间复杂度下运行。

# Example🕶

```
**Input:** [1,2,3,4,5]
**Output:** true**Input:** [5,4,3,2,1]
**Output:** false
```
...
1.  因为我们必须比较三个元素，如果给定的数组长度小于三。返回**假**🔚

# 算法👨‍🎓

1.  在这个问题中，我们必须找到三个递增顺序的元素。
2.  将三个变量初始化为整数最大值。
3.  对于每个索引，检查当前数字是否小于这些变量中的任何一个。如果它小于它，将变量更改为指向当前元素。
4.  重复这个过程，直到数组结束，如果这三个变量已经从它们的初始值改变，那么数组中存在一个递增的顺序。
5.  否则返回 False。🔚

# 复杂性分析

> 时间复杂度:O(N) 因为我们只遍历一次。
> **空间复杂度:O(1)** 由于我们只使用了三个附加变量。

...
> 原文：<https://blog.devgenius.io/how-to-solve-partitioning-a-palindrome-using-backtracking-d35ce1668165?source=collection_archive---------8----------------------->

## 第 31 天——100 天到 LinkedIn、雅虎、甲骨文

![](img/af0bfac7bec320707506c71dc3310d4c.png)

弗拉基米尔·哈尔塔科夫在 [Unsplash](https://unsplash.com/t/sustainability?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) 上的照片

*   出于免费的故事？这里是我的 [**好友链接。**](https://medium.com/@akshay_ravindran/day-10-binary-tree-zig-zag-traversal-2265eb879f63?sk=cfdacdbd13f72bb911ceca2b858d756d)
*   100 天到 LinkedIn，雅虎，甲骨文

# Introduction🛹

嘿，伙计们，今天是 LinkedIn 挑战 100 天的第 31 天。
...
给定一个字符串 *s* ，划分 *s* 使得划分的每个子串都是一个回文。
...
1.  你必须递归地使用 DFS 算法来回溯。
2.  你从指向同一个字符的开始指针和结束指针开始。
3.  对于开始和结束指针之间的每个字符组合，验证该字符串是否是回文。
4.  如果是回文，将子串添加到路径列表中。当结束指针到达字符串的末尾时，通过改变起点来重复。
5.  每当您到达字符串的末尾时，添加结果的路径。记住，只有当结果不为空时，才将列表添加到结果中。🔚

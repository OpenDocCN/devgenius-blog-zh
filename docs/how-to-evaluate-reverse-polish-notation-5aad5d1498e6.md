...
> 原文：<https://blog.devgenius.io/how-to-evaluate-reverse-polish-notation-5aad5d1498e6?source=collection_archive---------27----------------------->

## 第 15 天— 100 天去 LinkedIn、雅虎、甲骨文

![](img/84dd9e398f21c5a20c8ed72f1532a61c.png)

*   出于免费的故事？这里是我的 [**好友链接。**](https://medium.com/@akshay_ravindran/how-to-evaluate-reverse-polish-notation-5aad5d1498e6?sk=1a03b852b6b0d32bb038c318f675c345)
*   100 天到 LinkedIn，雅虎，甲骨文

# Introduction🛹

嘿，伙计们，今天是 LinkedIn 挑战 100 天的第 15 天。
...
有效的操作符有`+`、`-`、`*`、`/`。每个操作数可以是整数或其他表达式。

**注:**

*   两个整数的除法应向零方向截断。
*   给定的 RPN 表达式总是有效的。这意味着表达式将总是计算出一个结果，并且不会有任何被零除的操作。
...
**Input:** ["2", "1", "+", "3", "*"]
**Output:** 9
**Explanation:** ((2 + 1) * 3) = 9**Input:** ["4", "13", "5", "/", "+"]
**Output:** 6
...
1.  创建一个以相反顺序存储数字的堆栈。
2.  当你遇到一个接线员。从**堆中弹出**顶部的两个**元件。**
3.  我使用第二个数字作为第一个 pop，这样我就不必担心分别改变减法和除法的变量，因为顺序对这些运算很重要。
4.  创建一个**开关箱**，它将根据**操作器的**类型**动作。**
5.  对于每个开关情况，对两个变量执行操作，并将结果再次推入堆栈。
6.  遍历结束时，堆栈顶部的元素就是结果。🔚

...
> 原文：<https://blog.devgenius.io/how-to-find-all-disappeared-numbers-in-an-array-26e9180d4199?source=collection_archive---------10----------------------->

## 第 28 天——100 天到 LinkedIn、雅虎、甲骨文

![](img/66aa6c86fb72b0cce3c356277d6d003c.png)

[简·侯伯](https://unsplash.com/@jan_huber?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)在 [Unsplash](https://unsplash.com/s/photos/disappear?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) 上拍照

*   出于免费的故事？这里是我的 [**好友链接。**](https://medium.com/@akshay_ravindran/26e9180d4199?source=friends_link&sk=a885d5527a1ec91b0af0a1672e4554db)
*   100 天到 LinkedIn，雅虎，甲骨文

# Introduction🛹

嘿，伙计们，今天是 LinkedIn 挑战 100 天的第 28 天。
...
给定一个整数数组，其中 1≤a[I]≤*n*(*n*=数组的大小)，有些元素出现两次，有些出现一次。

找出[1， *n* ]中没有出现在这个数组中的所有元素。

你能在没有额外空间的情况下在 O( *n* )运行时完成吗？你可能认为返回的列表不算额外空间。
...
1.  如果给定的数组为空，那么我们必须返回一个空的列表。

# 算法👨‍🎓

1.  对于数组中的每个元素，将当前元素指向的索引处的值更改为负数。
2.  再次遍历数组，在当前元素为正数的地方查找索引，然后查找缺少的数字。
3.  把它添加到列表中。在遍历结束时返回列表🔚
